...
$installer->run(
    "
        ...
        CONSTRAINT `CONST_advancedinventory_product_id` FOREIGN KEY (`product_id`) REFERENCES `{$this->getTable('catalog_product_entity')}` (`entity_id`) ON DELETE CASCADE ON UPDATE CASCADE,
        CONSTRAINT `CONST_advancedinventory_localstock_id` FOREIGN KEY (`localstock_id`) REFERENCES `{$this->getTable('advancedinventory_product')}` (`id`) ON DELETE CASCADE ON UPDATE CASCADE
        ) ENGINE=InnoDB DEFAULT CHARSET=utf8;
 "    
);
