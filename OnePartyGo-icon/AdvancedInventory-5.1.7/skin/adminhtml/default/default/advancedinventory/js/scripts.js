...
var InventoryManager = {
    ...
    updateQty: function() {
        ...
        qty = 0;
        $$('#advancedinventory_stocks TABLE TR DIV[visibility!=hidden] INPUT.validate-number').each(
            function(i) {
    ...
    updateStocks: function(e) {
        ...
        if (e.ancestors()[0].hasClassName("PosQty")) {
            qty = 0;
            e.ancestors()[2].select(".PosQty INPUT").each(
                function(i) {
    ...
    updateRemainingStock: function(e) {
        ...
        total = 0;
        e.ancestors()[1].select('INPUT').each(
            function(i) {
    ...
    enableMultiStock: function(type, from) {
        ...
        switch (type) {
            case "inventory":
                ...
                i = 0
                $$('#advancedinventory_stocks TABLE TR').each(
                    function(tr) {
                    if (i)
            ...
            case "grid":
                ...
                if (status === 'enabled') {
                    ...
                    this.getRow(from).select('.PosQty').each(
                        function(i) {
                        i.update("-")
                        }
                    )
...
document.observe(
    ...
    document.observe(
        ...
        if (e.findElement('INPUT.choose_assignation')) {
            ...
            $$('#ai-scroller TABLE TR').each(
                function(tr) {

                if (i) {
                    qty = tr.readAttribute('qty')
                    ii = 0;
                    tr.select("INPUT.keydown").each(
                        function(input) {
